...
const Button = ({ text, alt }: { text: string; alt: boolean }) => {
  return (
    <button className="relative inline-flex items-center justify-start px-5 py-[15px] overflow-hidden font-bold rounded-xl group w-full">
      <span className={`${alt ? "bg-white" : "bg-primary"} w-full h-full absolute left-0 top-0`}></span>
      <span
        className={`${
          alt ? "bg-purple-600" : "bg-white"
      ...
      <span
        className={`${
          alt ? "text-bgDark group-hover:text-white" : "text-white group-hover:text-gray-900"
      ...
      <span
        className={`${
          alt ? "hover:border-purple-600 border-white" : "hover:border-white border-primary"
